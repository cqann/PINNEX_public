...
def setup_fibrois(meshname, args, mesh_sz, dot_size):
    ...
    # Assumes you have a txt module with read and write functions
    # and that you have already imported any necessary modules.
    
    # Mesh dimensions and read mesh data
    x, y, z = mesh_sz
    ...
    print(n_tissue_elems)
    target_count = int(args.fibrosis_vol_fraction * n_tissue_elems)
    
    # Choose a random center for the fibrosis sphere (in mesh units, multiplied as in your script)
    center_x = random.uniform(0, 1) * x * 1000
    while center_x < 1000:
        center_x = random.uniform(0, 1) * x * 1000
    
    center_y = random.uniform(0, 1) * y * 1000
    center_z = random.uniform(0, 1) * z * 1000

    # Initialize radius (starting with dot_size, can be tuned)
    radius = 0.1  
    fibrotic_tetra_set = set()
    
    # Set a maximum radius to avoid an infinite loop (e.g. the maximum mesh extent)
    max_radius = max(x, y, z) * 1000
    
    # Increase radius until we have at least the target number of tissue tetrahedra
    while radius <= max_radius:
        command = ["meshtool", "query", "idx",
                   "-msh=" + meshname,
                   f"-coord={center_x},{center_y},{center_z}",
                   f"-thr={radius}"]
        try:
            result = subprocess.run(command, capture_output=True, text=True, check=True)
            # Expecting output to contain "Vertex list:" followed by comma‐separated vertex IDs
            parts = result.stdout.split("Vertex list:")
            if len(parts) != 2:
                fibrotic_points = []
            else:
                fibrotic_points = [int(v) for v in parts[1].strip().split(",")]
        except subprocess.CalledProcessError as e:
            print("Error executing meshtool command:")
            print(e.stderr)
            return

        # Collect all tetrahedra connected to the found vertices
        candidate_tetras = set()
        for vertex in fibrotic_points:
            candidate_tetras.update(vertex_to_tetra.get(vertex, []))
        
        # Filter to include only tissue tetrahedra (etags != 0)
        fibrotic_tetras = [i for i in candidate_tetras if etags[i] != 0]

        if len(fibrotic_tetras) >= target_count:
            fibrotic_tetra_set = candidate_tetras  # Accept this sphere
            break
        # Increase the radius by 10%
        radius *= 1.1

    # Actual count of tissue tetrahedra found in our sphere
    actual_count = len([i for i in fibrotic_tetra_set if etags[i] != 0])
    percent_achieved = actual_count / target_count * 100

    # In case we have more than we need, trim the list to match the target count
    ...
    # Split fibrotic tetrahedra into two groups: for example, half remodeling (tag 2) and half ECM (tag 3)
    ...
    # Debug message: report what percent of the intended fibrotic volume was actually achieved.
    print(f"Debug: Fibrotic region contains {actual_count} tissue tetrahedra, which is {percent_achieved:.2f}% of the intended {target_count} elements.")
