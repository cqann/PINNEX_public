...
def run(args, job):
    ...
    # Add fibrosis
    setup_fibrois(meshname, args, (x, y, z), 3500)
...
def setup_gregions(args):
    ...
    g_reg += [
        ...
        "-gregion[1].num_IDs", 1,
        "-gregion[1].ID[0]", 2,
        "-gregion[1].CV.vel", CV_healthy * 0.1,
        ...
        "-gregion[2].num_IDs", 1,
        "-gregion[2].ID[0]", 3,
        "-gregion[2].CV.vel", CV_healthy * 0.1,
...
def setup_fibrois(meshname, args, mesh_sz, dot_size):
    ...
    fib_regs = 0
    while True:  # len(fibrotic_tetra_set) < args.fibrosis_vol_fraction * n_tissue_elems:
        if fib_regs >= 3:
            break
        ...
        thr_radius = random.uniform(0.7 * dot_size, 1.3 * dot_size)
...
def compute_ECG(ECG, num_leads, job, idExp, meshname=None):
    ...
    if write_coordinates:
        ...
        load_healthy = True
        if load_healthy:
            healthy_csv = os.path.join(healthy_ECG, "healthy_ECG_slab.csv")
