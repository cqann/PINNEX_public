...
def parser():
    ...
    # In the parser() function, add a new argument for specifying the fibrosis center:
    group.add_argument(
        "--fibrosis_center",
        type=float,
        nargs=4,
        metavar=('X', 'Y', 'Z', 'r'),
        default=None,
        help="Specify the center of the fibrosis blob in mesh units (mm). Example: --fibrosis_center 10 10 10"
    )
...
def run(args, job):
    ...
    # Add fibrosis
    setup_fibrois(meshname, args, (x, y, z), 500)
    ...
    stim = [
        ...
        "-stim[0].ptcl.npls", 1,
        "-stim[0].ptcl.start", args.stim_delay,
    ]
    ...
    IO_par = ["-spacedt", 1, "-timedt", 5.0]  # output intervals (ms)
    ...
    simID = job.ID
    cmd += ["-meshname", meshname, "-tend", args.duration, "-simID", simID, "-eik_solve", "3", "-output_level", "2"]
    if args.visualize:
    ...
    if args.ECG is not None:
        ...
        compute_ECG(args.ECG, num_leads, job, args.ID, meshname=meshname)
        return
...
def setup_fibrois(meshname, args, mesh_sz, dot_size):
    ...
    # Helper to get seed coordinates: use cmd-specified center if provided, otherwise random.
    def get_seed():
        if args.fibrosis_center is not None:
            # Convert from mm to microns.
            return (args.fibrosis_center[0] * x * 1000,
                    args.fibrosis_center[1] * y * 1000,
                    args.fibrosis_center[2] * z * 1000)
        else:
            return (random.uniform(0, 1) * x * 1000,
                    random.uniform(0, 1) * y * 1000,
                    random.uniform(0, 1) * z * 1000)

    while len(fibrotic_tetra_set) < args.fibrosis_vol_fraction * n_tissue_elems:
        fibrotic_seed_x, fibrotic_seed_y, fibrotic_seed_z = get_seed()
        # Only perform the rejection test if a fixed center was not specified.
        if args.fibrosis_center is None:
            if (fibrotic_seed_x**2 + (fibrotic_seed_y - y * 500)**2) < 4 * (y * 100)**2:
                continue

        if args.fibrosis_center is None:
            mean_target = dot_size
            low, high = mean_target * 0.5, mean_target * 2
            sigma = math.log(high / mean_target) / 2
            mu = math.log(mean_target) - (sigma ** 2) / 2
            thr_radius = random.lognormvariate(mu, sigma)
        else:
            thr_radius = args.fibrosis_center[3] * x * 1000

        command = [
            "meshtool", "query", "idx",
            "-msh=" + meshname,
            f"-coord={fibrotic_seed_x},{fibrotic_seed_y},{fibrotic_seed_z}",
            f"-thr={thr_radius}"
        ]
        try:
...
def writeECGgrid(meshname, mesh_sz, num_points, res, bath_size_factor):
    ...
    corners = [
        ...
        (x_len / 2, y_len / 2, z_len),
        (x_len / 2, y_len, z_len / 2),
        (x_len, y_len / 2, z_len / 2),
    ]
...
def compute_ECG(ECG, num_leads, job, idExp, meshname=None):
    if False:
        ...
        # Derive the healthy simulation folder from the provided ECG path.
        healthy_ECG = re.sub(r"vol\d+", "vol0", ECG)

        # Construct the path to the block.elem file inside the mesh folder.
        elem_file = meshname + ".elem"
        print(f"Looking for block.elem in: {elem_file}")
        if not os.path.exists(elem_file):
            raise FileNotFoundError(f"block.elem not found in the directory: {meshname}")

        # Read all lines from the file.
        with open(elem_file, "r") as f:
            lines = f.readlines()

        # Skip the first line (number of elements) and extract the last token (region number) from each subsequent line.
        region_numbers = [int(line.strip().split()[-1]) for line in lines[1:] if line.strip()]
        region_numbers = [0 if x == 1 else 1 for x in region_numbers]

        # Save the region numbers to a CSV file in the healthy simulation folder.
        fibrotic_csv = os.path.join(ECG, "fibrotic_ECG.csv")
        os.makedirs(os.path.dirname(fibrotic_csv), exist_ok=True)
        np.savetxt(fibrotic_csv, np.array(region_numbers), delimiter=",", fmt="%d")

        print(f"Region numbers saved to: {fibrotic_csv}")
        return
    import os
    import re
    import numpy as np
    ...
    healthy_ECG = "".join(ECG.split("/")[:-1]) + "healthy"
    ...
    healthy_leads_raw = np.loadtxt(healthy_csv, delimiter=",")
