...
def parser():
    ...
    group.add_argument(
        ...
        metavar=("X", "Y", "Z", "r"),
        default=None,
        help="Specify the center of the fibrosis blob in mesh units (mm). Example: --fibrosis_center 0.3 0.3 0.4 0.2",
    )
    group.add_argument(
...
def run(args, job):
    ...
    setup_fibrois(meshname, args, mesh_sz, 500)  # Fixa
    imp_reg = setup_ionic()
...
def setup_fibrois(meshname, args, mesh_sz, dot_size):
    ...
    def get_seed():
        if args.fibrosis_center is not None:
            ...
            return (
                (args.fibrosis_center[0] - 1 / 2) * x * 1000,
                (args.fibrosis_center[1] - 1 / 2) * y * 1000,
                (args.fibrosis_center[2] - 1 / 2) * z * 1000,
            )
        else:
            return (
                random.uniform(0, 1) * x * 1000,
    ...
    fibrotic_fast = [i for i in fibrotic_tetra_set if etags[i] == 0]
    fibrotic_slow = [i for i in fibrotic_tetra_set if etags[i] == 1]
    print("Found " + str(len(fibrotic_tetra_set)))

    etags[fibrotic_fast] = 2
    etags[fibrotic_slow] = 2
