...
ECG_ORDER = [
    "Lead I", "Lead II", "Lead III", "Lead IV", "Lead V", "Lead VI",
    "Lead VII", "Lead VIII"
]
...
def process_ecg(sim_id, ecg_csv_file, delay=0.0, ecg_mask=None, df_healthy=None):
    ...
    if LOAD_FROM_SETTINGS:
        ...
        parameters = [settings["fibrosis_center_x"], settings["fibrosis_center_y"],
                      settings["fibrosis_center_z"], settings["fibrosis_size"]]
...
def process_all_simulations(sim_root, out_dir):
    sim_folders = [d for d in os.listdir(sim_root) if os.path.isdir(os.path.join(sim_root, d))]
    activation_dfs = []
    ecg_dfs = []
    common_points_file = os.path.join(sim_root, "slab_04.pts")
    # df_healthy = pd.read_csv("healthy_slab_ecg.csv", header=None)
    ...
    for sim_id in sim_folders:
        ...
        ecg_csv_file = os.path.join(sim_folder, "fibrotic_ECG.csv")

        delay = 0
        if RANDOM_DELAY_ACTIVE:
            # delay = [0.0, DELAY_MAX / 3, 2 * DELAY_MAX / 3, DELAY_MAX][i % 4]
            delay = random.uniform(DELAY_MIN, DELAY_MAX)
        print(f"Using delay {delay:.4f} for simulation {sim_id}")

        data_sim_id = int(sim_id) * 10 + int(i) % 10 if RANDOM_DELAY_ACTIVE else int(sim_id)
        if RANDOM_DELAY_ACTIVE:
            delays.append((data_sim_id, delay))

        df_act = process_activation(data_sim_id, act_igb_file, common_points_file, delay=delay)
        activation_dfs.append(df_act)

        df_ecg = process_ecg(data_sim_id, ecg_csv_file, delay=delay, ecg_mask=ecg_mask, df_healthy=df_healthy)
        ecg_dfs.append(df_ecg)
        i += 1
        if i % 50 == 0:
            print("==============================================================")
            print(f"Processed {i/len(sim_folders):.2%} of simulations.")
            print("==============================================================")
    ...
    if RANDOM_DELAY_ACTIVE:
        # Write delays array to a text file in the output directory
        delays_file = os.path.join(out_dir, "delays.txt")
        with open(delays_file, "w") as f:
            for sim_id, delay in delays:
                f.write(f"{sim_id},{delay:.4f}\n")
        print(f"Delays written to {delays_file}")
...
def main():
    ...
    out_dir = "sim_parsed"     # Output directory for aggregated Parquet files
