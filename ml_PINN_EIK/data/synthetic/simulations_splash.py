...
def generate_stimulation_params(n_simulations=2):
    ...
    for i in range(n_simulations):
        n_patches = 0 if i % 100 == 0 else 1
        fibrosis_vol_fraction = 0
        params = {
            "sim_id": str(i + 1 + 1000),
            "fibrosis_vol_fraction": fibrosis_vol_fraction,       # keep or change as needed
            "seed": str(i + 1 + 1000),
            "n_patches": n_patches,
        }
...
def run_simulations(n_simulations=2):
    ...
    simulation_script = os.path.join(proto_dir, "ml_eik_slab_splash_realistic.py")
    ...
    for params in param_list:
        ...
        cmd = [
            ...
            "--n_patches", str(params["n_patches"]),
...
def main():
    run_simulations(n_simulations=11)  # or however many you'd like to run
