...
def generate_stimulation_params(n_simulations=2):
    ...
    for i in range(n_simulations):
        ...
        while True:
            center_x = random.uniform(0, 1)  # or whatever domain suits your problem
            center_y = random.uniform(0, 1)
            center_z = random.uniform(0, 1)

            if (center_x ** 2 + (center_y - 0.5) ** 2 + center_z ** 2) >= 0.25:
                break

        if random.uniform(0, 1) < 0.01:
            fibrosis_radius = 0
        else:
            fibrosis_radius = random.uniform(0.3, 0.4)  # Random size between 0.1 and 0.5 mm

        stim_delay = 0  # random.uniform(0, 20)
        params = {
            "sim_id": str(i + 1),
            "stim_delay": stim_delay,                  # or random.uniform(...)
            "fibrosis_vol_fraction": 0.3,       # keep or change as needed
...
def run_simulations(n_simulations=2):
    ...
    proto_dir = os.path.abspath(
        os.path.join(current_dir, "..", "..", "..", "prototype", "ml_EIK")
    )
    simulation_script = os.path.join(proto_dir, "ml_eik_fibrosis_blob.py")

    # List of file names to be collected
    file_list = ["act.igb", "fibrotic_ECG.csv"]
    ...
    for params in param_list:
        ...
        cmd = [
            ...
            "--fibrosis_center", str(params["fibrosis_center_x"]), str(params["fibrosis_center_y"]
                                                                       ), str(params["fibrosis_center_z"]), str(params["fibrosis_size"]),
        ]
        print(" ".join(cmd))
...
def main():
    run_simulations(n_simulations=5)  # or however many you'd like to run
