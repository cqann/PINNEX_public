...
def prepare_openCarp(folder, heart, file_name):
    ...
    np.savetxt(
        pts_output, pts * 1000.0, header=str(pts.shape[0]), comments="", fmt="%.1f"
    )
    ...
    with open(csv_output, "w") as out:
        ...
        for i in range(npoints):
            out.write(
                f"{x[i]},{y[i]},{z[i]},{ab_vals[i]},{rt_vals[i]},{tm_vals[i]},{tv_vals[i]}\n"
            )

    cobi_out = os.path.join(output_folder, "cobi.csv")
    with open(csv_output) as infile, open(cobi_out, "w", newline="") as outfile:
...
def extract_fiber_sheet(vtk_file: str, output_file: str):
    ...
    fiber_array = data.GetCellData().GetArray("Fiber") or data.GetPointData().GetArray(
        "Fiber"
    )
    sheet_array = data.GetCellData().GetArray("Sheet") or data.GetPointData().GetArray(
        "Sheet"
    )
    if fiber_array is None or sheet_array is None:
        print("Fiber/Sheet array not found.")
        sys.exit(1)
    ...
    with open(output_file, "w") as f:
        ...
        for i in range(fiber_array.GetNumberOfTuples()):
            ...
            f.write(
                f"{fiber[0]:.6f} {fiber[1]:.6f} {fiber[2]:.6f} {sheet[0]:.6f} {sheet[1]:.6f} {sheet[2]:.6f}\n"
            )


def run_set_fibers(folder: str = "Mean", heart: str = "Original"):
    base_dir = os.path.dirname(__file__)
    target_folder = os.path.abspath(
        os.path.join(base_dir, "..", "Meshes", heart, folder)
    )
