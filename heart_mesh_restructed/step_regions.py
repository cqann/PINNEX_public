...
def run_set_regions(folder: str = "Mean") -> None:
    ...
    if not os.path.exists(csv_file):  print(f"CSV missing: {csv_file}");  sys.exit(1)
    if not os.path.exists(elem_file): print(f"Elem missing: {elem_file}"); sys.exit(1)
    ...
    rows = get_rows_within_coordinates(csv_file, ab_bounds, rt_bounds, tm_bounds, tv_bounds)
    mask = write_mask_dat_for_csv(csv_file, rows, os.path.join(target_folder,"mask.dat"))
...
def update_elem_file_quick(elem_filename, row_indices, region_id, output_filename=None, reset_all=False):
    ...
    if in_place:
        tmp = tempfile.NamedTemporaryFile('w', delete=False); output_filename = tmp.name; tmp.close()
    with open(elem_filename) as infile, open(output_filename,"w") as out:
        header = next(infile, None); out.write(header)
        for line in infile:
            parts = line.strip().split()
            if not parts: out.write("\n"); continue
            if reset_all: parts[-1] = "0"
            else:
                if any(int(tok) in row_indices for tok in parts[1:-1]):
                    parts[-1] = str(region_id)
            out.write(" ".join(parts)+"\n")
    if in_place: os.replace(output_filename, elem_filename)
